...
");
    ...
    /// Returns true if server-side truncation succeeded; false if skipped or failed.
    /// </summary>
    public async Task<bool> ClearHistoryAsync(string name)
    {
        ...
        // Attempt server-side truncation if we have a live SDK session with a valid SessionId
        ...
        if (!string.IsNullOrEmpty(state.Info.SessionId))
        {
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ClearHistory] Chat database clear failed for '{name}': {ex.Message}");
            }
        }
