    ...
    public static readonly GroupPreset[] BuiltIn = new[]
    {
            ...
            "claude-opus-4.6", new[] { "claude-sonnet-4.6", "claude-sonnet-4.6", "claude-sonnet-4.6" })
        {
            ...
            RoutingContext = """
                ...
                ```
                @worker:Skill Validator-worker-3
                Your task: [detailed instructions]
                @end
                ```

                You MUST use the FULL worker session name (e.g., "Skill Validator-worker-3", not just "worker-3").
                Each response MUST contain @worker:/@end blocks. Text outside the blocks is your planning notes.

                ### Worker Names
                - **Skill Validator-worker-1** = Dotnet Skill Validator (runs `skill-validator` tool)
                - **Skill Validator-worker-2** = Anthropic Skill Evaluator (prompt analysis via Claude Code)
                - **Skill Validator-worker-3** = Eval Generator (creates tests/eval.yaml)

                ### 2-Phase Dispatch

                **Phase 1 — Always dispatch worker-3 first:**

                Write a single @worker block for worker-3. Tell it the skill directory path and to generate tests/eval.yaml.
                Do NOT check if eval.yaml exists yourself — worker-3 will check and skip if it already exists.
                ...
                **Phase 2 — After worker-3 completes, dispatch workers 1 and 2 together:**

                Write TWO @worker blocks in a single response:
                ```
                @worker:Skill Validator-worker-1
                Evaluate the skill at: <path>
                Eval.yaml status: [generated by worker-3 / pre-existing]
                @end

                @worker:Skill Validator-worker-2
                Evaluate the skill at: <path>
                Eval.yaml status: [generated by worker-3 / pre-existing]
                @end
                ```
